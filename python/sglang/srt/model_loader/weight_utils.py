...
def _find_local_hf_snapshot_dir(
    ...
    """Find local HF snapshot directory.

    IMPORTANT: This function is intended for CI environments only.
    The caller is responsible for gating calls with is_in_ci() checks.
    Caller MUST also hold the model lock before calling this function
    ...
    Returns:
        Path to local snapshot if found and contains weight files, None otherwise
    """
...
def download_weights_from_hf(
    ...
    with get_lock(model_name_or_path, cache_dir):
        # In CI: check for valid local cache first, validate, and cleanup if needed
        if is_in_ci():
            path = _find_local_hf_snapshot_dir(
                model_name_or_path, cache_dir, allow_patterns, revision
            )
            if path is not None:
                # Validate the local cache using consolidated validation
